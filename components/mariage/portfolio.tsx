...
export function MariagePortfolio({ data }: { data?: PortfolioData }) {
  ...
  return (
    <MotionConfig reducedMotion="user">
      <section id="portfolio-mariage" className="relative bg-cream py-32 sm:py-40">
        <div className="mx-auto max-w-[1320px] px-6 sm:px-14">
          ...
          <div className="space-y-32 sm:space-y-40">
            {data.items.map((c, idx) => {
              ...
              const badge = c?.badge ?? `Cas n°${String(idx + 1).padStart(2, "0")}`;
              ...
              return (
                <article key={c?._id ?? idx} className="relative">
                  ...
                  >
                      ...
                      {moodBoard.length > 0 && (
                        <div className="mt-5 grid grid-cols-3 gap-3 sm:gap-4">
                          {moodBoard.slice(0, 3).map((img, i) => {
                            const url = img?.asset
                              ? urlForImageString(img, { width: 600, quality: 85 })
                              : null;
                            if (!url) return null;
                            return (
                              <motion.div
                    ...
                    >
                      <div className="mb-5 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.28em] text-bordeaux">
            ...
            >
              ...
              {footerCta && (
                <a
                  ...
                  className="group inline-flex items-center gap-2 rounded-full border border-ink px-9 py-4 font-sans text-[12px] font-medium uppercase tracking-[0.22em] text-ink transition-all hover:-translate-y-0.5 active:translate-y-0 hover:bg-ink hover:text-cream"
                >
                  {footerCta.label}
