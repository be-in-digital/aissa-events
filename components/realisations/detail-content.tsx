...
export function RealisationDetailContent({
  realisation,
}: {
  realisation: NonNullable<RealisationBySlugQueryResult>;
}) {
  const hasDescription = Boolean(realisation.description);
  const hasStory = Boolean(realisation.story);
  if (!hasDescription && !hasStory && (!realisation.tags || realisation.tags.length === 0)) {
    return null;
  }
      ...
      <div className="mx-auto max-w-[760px] px-6 sm:px-14">
        {hasDescription ? (
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-80px" }}
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
            className="space-y-5 font-serif text-[17px] leading-[1.8] text-ink-soft sm:text-[18px]"
          >
            {realisation.story?.split("\n").filter(Boolean).map((para, i) => (
              <p key={i}>{para}</p>
            ))}
          </motion.div>
        ) : null}
